...
class PromptBuilder:
    ...
    def format_few_shot_examples(self) -> str:
        ...
        Prefer structured output (JSON) over prose answer to teach the model the expected format.
        """
        if not self.few_shot_examples:
            return ""
        
        examples_text = ["# أمثلة على الحالات المشابهة (التنسيق المطلوب):\n"]
        ...
        for i, example in enumerate(self.few_shot_examples):
            ...
            # Prefer structured output (JSON) to show the exact format expected
            if output and isinstance(output, dict) and 'heirs' in output:
                answer_part = "```json\n" + json.dumps(output, ensure_ascii=False, indent=2) + "\n```"
            elif '<answer>' in answer:
                answer_part = answer.split('<answer>')[1].split('</answer>')[0].strip()
                # If it looks like prose, try to get JSON from output
                if output and 'heirs' in output:
                    answer_part = "```json\n" + json.dumps(output, ensure_ascii=False, indent=2) + "\n```"
            else:
                answer_part = answer[:500] if answer else ""
            
            examples_text.append(f"\n## مثال {i+1}:\n")
            examples_text.append(f"السؤال: {question}\n")
            examples_text.append(f"الجواب (JSON): {answer_part}\n")
...
"""
    ...
    def build_prompt(
        ...
        prompt_parts.append(
            "**إلزامي:** ابدأ بـ <think> واكتب التفكير المفصل، ثم أغلق بـ </think> "
            "ثم ابدأ <answer> واكتب JSON المنظم بالضبط (heirs, blocked, shares, tasil_stage, awl_or_radd, post_tasil). "
            "لا تترك <answer> فارغاً أبداً.\n"
        )
