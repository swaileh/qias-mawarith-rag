...
class QIAS2026Evaluator:
    ...
    def evaluate_sample(self, prediction: Dict[str, Any], ground_truth: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Official MIR-E weights (5-component aggregation)
        # Ref: QIAS 2026 official evaluation protocol
        # Note: tasil_stage (awl_stage) is evaluated for diagnostics
        #       but NOT separately weighted in the MIR-E aggregate.
        mir_e_weights = {
            'heirs_accuracy': 0.30,       # λ₁ — eligible heirs
            'blocked_accuracy': 0.20,     # λ₂ — blocked heirs
            'shares_accuracy': 0.20,      # λ₃ — legal shares
            'awl_radd_accuracy': 0.10,    # λ₄ — ʿawl / radd
            'post_tasil_accuracy': 0.20,  # λ₅ — final distribution
        }

        results['overall_accuracy'] = sum(
            results[metric] * weight
            for metric, weight in mir_e_weights.items()
        )
    ...
    @staticmethod
    def _parse_fraction_value(frac_str: str) -> Optional[float]:
        """Parse a fraction string like '1/6' to a float value."""
        if not frac_str or not isinstance(frac_str, str):
            return None
        frac_str = frac_str.strip()
        # Arabic residuary terms cannot be compared numerically
        if frac_str in ('الباقي', 'باقى التركة', 'الباقى', 'تعصيب'):
            return None
        try:
            if '/' in frac_str:
    ...
    def _evaluate_shares_list(self, pred_shares: List[Dict], gt_shares: List[Dict],
                              epsilon: float = 0.01) -> float:
        """Evaluate shares using value-based indicator matching (official S_value).

        For each gold share entry, finds the matching predicted entry by
        (heir, count) and compares the fraction value with tolerance ε.
        Returns the mean per-entry indicator score.
        """
        ...
        # Build prediction lookup by (heir_name, count)
        pred_lookup: Dict[tuple, str] = {}
        for s in pred_shares:
            key = (s.get('heir', '').strip(), s.get('count', 0))
            pred_lookup[key] = s.get('fraction', '')

        # Evaluate each gold entry with S_value
        scores: List[float] = []
        for s in gt_shares:
            key = (s.get('heir', '').strip(), s.get('count', 0))
            gt_frac = s.get('fraction', '')

            if key not in pred_lookup:
                scores.append(0.0)
                continue

            pred_frac = pred_lookup[key]

            # Numeric comparison with tolerance ε
            gt_val = self._parse_fraction_value(gt_frac)
            pred_val = self._parse_fraction_value(pred_frac)

            if gt_val is not None and pred_val is not None:
                scores.append(1.0 if abs(gt_val - pred_val) <= epsilon else 0.0)
            else:
                # Fallback: exact string match for non-numeric values
                scores.append(1.0 if gt_frac.strip() == pred_frac.strip() else 0.0)
