...
class OutputParser:
    ...
    def parse(self, model_output: str) -> Dict[str, Any]:
        ...
        json_str = self._extract_json(answer if answer else model_output)
        if json_str:
            try:
        ...
        if not result['parsing_success']:
            ...
            if structured:
                ...
                # Try to extract structured data from the thinking text itself
    ...
    def format_for_evaluation(self, parsed_output: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'output': parsed_output.get('structured_output', {}),
        }
